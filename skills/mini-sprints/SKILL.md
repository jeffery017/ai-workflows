...
任務來自一個**工作單元**（work unit）——Jira ticket、plan/設計文件、或一段已討論過
的需求。把 requirement → design → implementation 拆成刻意限縮的小迭代（mini-sprint），
用檔案結構**模擬** sprint：每一刀只做一個有意義的邊界，讓使用者 review 時認知負擔最小。

你不是在「規劃完整方案然後執行」，而是在「選一個小切片、做、收集回饋、再規劃下一刀」。
抗拒一次做完的衝動。整體需求可以模糊，但每一刀的 scope 必須清晰。

（設計動機、要解決的瀑布式痛點、各項取捨的完整論述在 `README.md`；本檔只給執行指令。）
...
**漸進式定義需求 —— 這是核心精神。** 不要求一次把需求定義到 100%：`requirements.md`
是**活文件**，允許 Core / Derived Spec 帶著「尚未釐清」的留白，隨 iteration 補完。真正
需要**語意精準、覆蓋完整**的是每個 iteration 的 `scope.md` —— 那是即將動手的確定範疇。

**但漸進式只放寬「需求細節」，不放寬「整體架構」。** 拆 scope、定義細部規格前，必須先
對整體的架構與流程有完整的設計掌握（需要時落成 `specs/design.md`）。缺了共同的架構骨架，
各刀會各切各的、事後難以整合——漸進式收斂的是細節，不是拿來迴避架構設計。
...
**任務夠小就別硬拆。** 若整個工作單元範圍單純、小到一刀就能做完並乾淨 review，就用
一個 iteration 完成，不必為了拆而拆——mini-sprint 的價值在降低 review 認知負擔，範圍本
來就小時多切只是徒增儀式。
